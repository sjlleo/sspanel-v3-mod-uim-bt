...
sed -i 's/system,//g' /www/server/php/74/etc/php.ini
sed -i 's/proc_open,//g' /www/server/php/74/etc/php.ini
sed -i 's/proc_get_status,//g' /www/server/php/74/etc/php.ini
sed -i 's/putenv,//g' /www/server/php/74/etc/php.ini
sed -i 's/dynamic/static/g' /www/server/php/74/etc/php-fpm.conf
sed -i 's/display_errors = On/display_errors = Off/g' /www/server/php/74/etc/php.ini
...
echo -e "${Info} 如果打不开站点，请到宝塔面板中软件管理重启nginx和php7.4"
